...
program
  ...
  .option(
    ...
    "The effective period, CURRENT, FUTURE or ALL"
  )
  .description(
    "Returns the complete product array for a given bank and given filters"
  )
...
program
  ...
  .description(
    "Calls the requested bank's product API returning data based on filters "
  )
  .action(async (bank, xv, options) => {
    let inputOptions = [];
    if (options.productCategory) {
...
program
  ...
  .action(async (bank, xv, productID, options) => {
    ...
    if (options.xMinV) {
      try {
        res = await getProductDetails.getProductDetails(
          bank,
          xv,
          productID,
          options.xminv

...
async function callGetProductsApi(bank, xv) {
    ...
    options.forEach(option => {
        ...
        if (Object.keys(option)[0] === 'x-min-v') { // If the optional value is xvmin, assign it to xvmin
 ...
 */
async function getProductsArray(bank, xv, options) {
    let productArray = new Array
    let pageNo = 1;
    let numPages;
    let params = []
    params.push(bank); params.push(xv)
 ...
 *  Either way, returns just an array of the options 
 * @param {Array} args 
 * @return {Array}
 */
